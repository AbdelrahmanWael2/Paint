...
export class AppComponent {
  ...
  Undo = false;
		   	...
		   	this.SelectedItem.on("transformend", (e: any) =>{
				this.paintService.getObj(this.SelectedItem.id()).subscribe((res : any) => 
				{
					...
					switch(shape.name){
						...
						break;
						case "line": 
							// shape.points[2] = shape.points[2] * this.SelectedItem.scaleX()*Math.cos(this.SelectedItem.rotation);
							// shape.points[3] = shape.points[3] * this.SelectedItem.scaleX()*Math.sin(this.SelectedItem.rotation);
							// var newpos = this.SelectedItem.getAbsoluteTransform().point({ x: 0, y: 0})
							this.scalePoints(shape);
							// console.log("points " + this.SelectedItem.points)
	...
	scalePoints(shape: IShape){
		const oldPoints = this.SelectedItem.points();
		const newPoints = [];
		for(var i = 0; i< oldPoints.length / 2; i++) {
			const point = this.SelectedItem.getTransform().point({
				x: oldPoints[i * 2],
				y: oldPoints[i * 2 + 1]
				
			})
			
			newPoints.push(point.x - this.SelectedItem.x(), point.y - this.SelectedItem.y());
			console.log(point.x - this.SelectedItem.x() )
			console.log( point.y - this.SelectedItem.y());
		}
		this.SelectedItem.points(newPoints);
	...
	renderingData(Arr: IShape[]){
		for( let i = 0; i < Arr.length ; i += 1 ){
			switch( Arr[i].name ){
				...
				case "line": this.drawLine( Arr[i] ); 
				console.log("rotstion" + Arr[i].rotation)
				break;
